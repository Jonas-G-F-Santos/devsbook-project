...
class PostDaoMysql implements PostDAO{
    ...
    public function getHomeFeed($id_user, $page = 1){
        ...
        // 2. Me adicionar na lista para pegar os meus posts também.
        $userList[] = $id_user;
        // 3. Pegar os posts ordenado pela data
        $sql = $this->pdo->query("SELECT * FROM posts
        WHERE id_user IN (".implode(',', $userList).")
        ORDER BY created_at DESC, id DESC LIMIT $offset,$perPage");
        if($sql->rowCount() > 0) {
            $data = $sql->fetchAll(PDO::FETCH_ASSOC);
            // 4. Transformar o resultado em objetos
            $array['feed'] = $this->_postListToObject($data, $id_user);
        }
        // 5. Pegar o TOTAL de posts
